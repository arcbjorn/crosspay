<script lang="ts">
	...
	async function loadTokenBalances() {
		...
		try {
			tokenBalances = await erc20Service.getTokenBalances(
				supportedTokens,
...
<div class="form-control">
	<label class="label" for="token-selector">
		...
		<span class="label-text-alt">
			...
			{:else if selectedTokenInfo}
				Balance: {getTokenBalance(selectedToken)?.formattedBalance || '0'}
	...
	{#if selectedTokenInfo}
		<div class="bg-base-200 mt-2 rounded-lg p-3">
			<div class="flex items-center gap-3">
				...
				/>
				<div class="flex-1">
					<div class="text-sm font-medium">{selectedTokenInfo.name}</div>
