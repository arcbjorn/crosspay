...
<CyberCard variant="danger" padding="lg">
	<div class="terminal-text mb-6 text-xl text-cyber-error">[RISK_ASSESSMENT_AI]</div>

	{#if loading}
		<div class="flex flex-col items-center py-8">
			<div class="ascii-loader mb-4 text-cyber-mint">
...
`}
	...
	{:else}
		<!-- ASCII Risk Meter -->
		<div class="terminal-text mb-6 text-center">
			<div class="mb-2 text-sm text-cyber-text-secondary">RISK_SCORE:</div>
			<div class="{riskColor} mb-2 font-mono text-3xl">
			...
			<div class="mt-4 font-mono text-xs">
				<div class="border border-cyber-border-mint bg-cyber-bg-secondary p-2">
		...
		{#if factors.length > 0}
			<div class="border-t border-cyber-border-mint/20 pt-4">
				<div class="terminal-text mb-3 text-sm text-cyber-text-secondary">
					...
					{#each factors as factor}
						<div class="cyber-card bg-cyber-surface-2 p-3">
							<div class="mb-2 flex items-center justify-between">
								<span class="terminal-text text-sm text-cyber-text-primary">
									{factor.name.toUpperCase()}
								</span>
								<span
									class="terminal-text {factor.impact > 20
										? 'text-cyber-error'
										: factor.impact > 0
											? 'text-cyber-warning'
											: factor.impact > -20
												? 'text-cyber-mint'
												: 'text-cyber-success'} text-sm"
								>
							...
							<!-- Impact Bar -->
							<div class="relative mb-2 h-2 bg-cyber-text-tertiary/20">
								<div
									class="absolute left-1/2 h-full {factor.impact > 0
										? 'bg-cyber-error'
										: 'bg-cyber-success'}"
									style="width: {Math.abs(factor.impact)}%; {factor.impact > 0
		...
		<!-- Recommendations -->
		<div class="mt-4 border-t border-cyber-border-mint/20 pt-4">
			<div class="terminal-text mb-2 text-sm text-cyber-text-secondary">AI_RECOMMENDATION:</div>
