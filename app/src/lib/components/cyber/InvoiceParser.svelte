<script lang="ts">
  ...
  import { invoiceParserAI, type ParsingResult, type InvoiceData } from '$lib/services/ai/invoiceParser';
...
<CyberCard variant="lavender" padding="lg">
  <div class="terminal-text text-cyber-lavender text-xl mb-6">
    [INVOICE_PARSER_AI]
  </div>
  ...
  {#if !result}
    ...
    >
      {#if parsing}
        <div class="space-y-4">
          ...
          <div class="terminal-text text-cyber-lavender">
            ...
            <div class="font-mono text-sm text-cyber-text-secondary mb-2">
              {progressMessage}
            </div>
            
            <!-- ASCII Progress Bar -->
            <div class="bg-cyber-bg-secondary border border-cyber-border-lavender p-2">
            ...
            <div class="text-xs text-cyber-text-tertiary mt-2">
      ...
      {:else}
        <div class="space-y-4">
          <!-- Upload Icon -->
          <div class="terminal-text text-cyber-lavender text-4xl">
            ┌─────────────────┐<br>
            │ ┌─────────────┐ │<br>
            │ │   INVOICE   │ │<br>
            │ │ ░░░░░░░░░░░ │ │<br>
          ...
          <div class="terminal-text text-cyber-text-primary">
          ...
          <div class="flex gap-4 justify-center">
            <CyberButton variant="secondary" on:click={() => document.getElementById('file-input')?.click()}>
              [BROWSE_FILES]
            </CyberButton>
          </div>
  ...
  {:else}
    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="terminal-text text-cyber-text-tertiary text-sm">
      ...
      {#if result.success && result.data}
        ...
        <div class="cyber-card bg-cyber-surface-2 p-4">
          <div class="terminal-text text-cyber-mint mb-4">EXTRACTED_DATA:</div>
          
          <div class="grid md:grid-cols-2 gap-6">
            <!-- Header Info -->
            <div class="space-y-2">
              <div class="terminal-text text-cyber-text-secondary text-sm">INVOICE_DETAILS:</div>
            ...
            <!-- Vendor Info -->
            <div class="space-y-2">
              <div class="terminal-text text-cyber-text-secondary text-sm">VENDOR:</div>
              <div class="text-xs">
                <div class="text-cyber-text-primary font-mono">
                  {result.data.vendor?.name || 'Unknown'}
                </div>
                {#if result.data.vendor?.address}
                  <div class="text-cyber-text-tertiary mt-1">
          ...
          {#if result.data.items.length > 0}
            <div class="mt-6">
              <div class="terminal-text text-cyber-text-secondary text-sm mb-3">LINE_ITEMS:</div>
              
              <div class="cyber-table-container text-xs">
                <table class="w-full">
                  <thead>
                    <tr class="border-b border-cyber-border-lavender">
                      <th class="text-left py-2 px-3 text-cyber-text-primary">DESCRIPTION</th>
                      <th class="text-right py-2 px-3 text-cyber-text-primary">QTY</th>
                      <th class="text-right py-2 px-3 text-cyber-text-primary">RATE</th>
                      <th class="text-right py-2 px-3 text-cyber-text-primary">TOTAL</th>
                    </tr>
                  </thead>
                  <tbody>
                    {#each result.data.items as item}
                      <tr class="border-b border-cyber-text-tertiary/20">
                        <td class="py-2 px-3 text-cyber-text-secondary">{item.description}</td>
                        <td class="py-2 px-3 text-cyber-text-secondary text-right font-mono">{item.quantity}</td>
                        <td class="py-2 px-3 text-cyber-text-secondary text-right font-mono">
                          {formatCurrency(item.unitPrice)}
                        </td>
                        <td class="py-2 px-3 text-cyber-text-primary text-right font-mono">
          ...
          <div class="mt-6 flex justify-end">
            <div class="space-y-1 text-sm min-w-48">
              {#if result.data.subtotal}
                <div class="flex justify-between">
                  <span class="text-cyber-text-tertiary">SUBTOTAL:</span>
                  <span class="text-cyber-text-primary font-mono">{formatCurrency(result.data.subtotal)}</span>
                </div>
              {/if}
              {#if result.data.tax}
                <div class="flex justify-between">
                  <span class="text-cyber-text-tertiary">TAX:</span>
                  <span class="text-cyber-text-primary font-mono">{formatCurrency(result.data.tax)}</span>
                </div>
              {/if}
              <div class="flex justify-between border-t border-cyber-border-lavender pt-1">
                <span class="text-cyber-lavender font-bold">TOTAL:</span>
                <span class="text-cyber-lavender font-mono font-bold">{formatCurrency(result.data.total)}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {#if result.errors.length > 0}
        <div class="cyber-card border-cyber-error p-4">
          <div class="terminal-text text-cyber-error mb-2">PARSING_ERRORS:</div>
      ...
      <div class="flex gap-4">
        ...
        <CyberButton variant="secondary" on:click={clearResults}>
          [PARSE_ANOTHER]
        </CyberButton>
      </div>
    </div>
