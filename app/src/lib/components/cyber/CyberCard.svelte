<script lang="ts">
  ...
  $: variantClass = variant === 'mint' ? 'border-cyber-border-mint' :
                    variant === 'lavender' ? 'border-cyber-border-lavender' :
                    variant === 'danger' ? 'border-cyber-error' :
                    'border-cyber-border-mint';
  
  $: paddingClass = padding === 'sm' ? 'p-3' :
                    padding === 'lg' ? 'p-8' :
                    'p-6';
...
<div class={cardClasses} {...$$restProps}>
  {#if matrix}
    <div class="security-matrix absolute top-2 right-2 opacity-20">
      {#each Array(9) as _, i}
        <div class="matrix-cell w-2 h-2 {i % 3 === 0 ? 'active' : ''}"></div>
      {/each}
    </div>
  {/if}
