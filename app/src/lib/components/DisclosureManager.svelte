...
<div class="disclosure-manager">
  ...
  {#if !isPrivate}
    <div class="public-notice">
      🔓 This payment is public - amounts are visible to all
    </div>
  {:else}
    <div class="private-notice">
      🔒 This payment is private - amounts are encrypted
    </div>
        ...
        {#each disclosureRequests as request}
          <div class="request-card" class:approved={request.approved}>
            <div class="request-header">
              <span class="requester">From: {request.requester.slice(0,8)}...{request.requester.slice(-6)}</span>
...
<style>
  ...
  .status-disclosed { background: #cce5ff; color: #004085; }

  .request-reason, .request-time {
    margin-bottom: 0.5rem;
    font-size: 0.9rem;
  }
