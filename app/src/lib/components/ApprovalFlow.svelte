<script lang="ts">
	...
	async function handleApprove() {
		...
		try {
			...
			let hash: string;
			if (approvalType === 'max') {
				hash = await erc20Service.approveMax(
					tokenAddress,
					spenderAddress,
					wallet.address as Address
				);
...
{#if show}
	<div class="modal modal-open">
		<div class="modal-box">
			<h3 class="mb-4 text-lg font-bold">Token Approval Required</h3>
			...
			{:else if approvalStatus}
				<div class="space-y-4">
					...
					<div class="alert alert-info">
						...
						<div>
							<div class="mb-1 font-medium">Why do I need to approve?</div>
							<div class="text-sm">
								You need to give permission for the CrossPay contract to spend your {tokenInfo?.symbol}
								tokens. This is a one-time transaction that enables token payments.
							</div>
						</div>
					</div>
			...
			<div class="modal-action">
				<button class="btn btn-ghost" on:click={handleCancel} disabled={isApproving}>
					Cancel
				</button>
