<script lang="ts">
	...
	onMount(async () => {
		...
		unsubscribe = oracleService.subscribeToPriceUpdates(symbol, (newPrice) => {
			if (price) {
				...
				priceDirection =
					newPrice.price > previousPrice ? 'up' : newPrice.price < previousPrice ? 'down' : 'same';
			}
			price = newPrice;
			loading = false;
		});
	...
	$: priceChangeClass =
		priceDirection === 'up' ? 'text-success' : priceDirection === 'down' ? 'text-error' : '';
		...
		<div class="card-body p-4">
			...
			{:else if error}
				<div class="text-error">
					...
					<button class="btn btn-ghost btn-xs mt-2" on:click={loadPrice}> Retry </button>
				</div>
			{:else if price}
				<div class="mt-2">
					<div class="font-mono text-2xl font-bold {priceChangeClass}">
						${oracleService.formatPrice(price.price, price.decimals || 2)}
					</div>
			...
			<div class="card-actions mt-2 justify-end">
				<button class="btn btn-ghost btn-xs" on:click={loadPrice} disabled={loading}>
