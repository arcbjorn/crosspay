...
<CyberModal bind:open={showControls} title="ACCESSIBILITY_CONTROLS">
	<div class="space-y-6">
		...
		<div class="cyber-card bg-cyber-surface-2 p-4">
			<div class="terminal-text mb-4 text-sm text-cyber-mint">[VISUAL_SETTINGS]</div>

			<div class="space-y-3">
				<label class="flex items-center justify-between">
					<span class="terminal-text text-sm text-cyber-text-primary"> HIGH_CONTRAST_MODE </span>
					<button
				...
				<label class="flex items-center justify-between">
					<span class="terminal-text text-sm text-cyber-text-primary"> LARGER_TEXT_SIZE </span>
					<button
						class="accessibility-toggle-btn {settings.largerText ? 'active' : ''}"
						on:click={() => {
							toggleSetting('largerText');
							announceToScreenReader(
								'Large text ' + (settings.largerText ? 'enabled' : 'disabled')
							);
						}}
		...
		<!-- Motion Settings -->
		<div class="cyber-card bg-cyber-surface-2 p-4">
			<div class="terminal-text mb-4 text-sm text-cyber-mint">[MOTION_SETTINGS]</div>

			<div class="space-y-3">
				<label class="flex items-center justify-between">
					<span class="terminal-text text-sm text-cyber-text-primary"> REDUCE_ANIMATIONS </span>
					<button
			...
			<div class="terminal-text mt-2 text-xs text-cyber-text-tertiary">
		...
		<!-- Navigation Settings -->
		<div class="cyber-card bg-cyber-surface-2 p-4">
			<div class="terminal-text mb-4 text-sm text-cyber-mint">[NAVIGATION_SETTINGS]</div>
				...
				<label class="flex items-center justify-between">
					...
					<button
						...
						on:click={() => {
							...
							announceToScreenReader(
								'Keyboard navigation indicators ' + (settings.keyboardNav ? 'enabled' : 'disabled')
							);
						}}
		...
		<!-- Keyboard Shortcuts -->
		<div class="cyber-card bg-cyber-surface-2 p-4">
			<div class="terminal-text mb-4 text-sm text-cyber-mint">[KEYBOARD_SHORTCUTS]</div>
