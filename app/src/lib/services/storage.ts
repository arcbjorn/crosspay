...
export class StorageService {
	...
	constructor(baseUrl?: string) {
		this.baseUrl = (baseUrl || import.meta.env.VITE_STORAGE_URL || 'http://localhost:3001').replace(
			/\/$/,
			''
	...
	async uploadReceipt(receiptData: any, paymentId: string): Promise<UploadResult> {
		...
		return this.uploadData(data.buffer, `receipt-${paymentId}.json`, 'application/json');
	}
